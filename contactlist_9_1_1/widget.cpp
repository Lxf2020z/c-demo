...
{
    ui->setupUi(this);
    //定义4个联系人，添加到列表对象和列表控件一直同步
    //第一个
    Contact daala("Daala","4444","North");      //实例化一个联系人的对象
    m_listContact.append(daala);                //把该对象放到QList列表中
    ui->listWidget->addItem(daala.toString());  //同时在窗口的 列表控件中展示
    //第2个
    Contact bob("Bob", "22222", "South");
    m_listContact.append(bob);
    ui->listWidget->addItem( bob.toString() );
    //第3个
    Contact carl("Carl", "33333", "West");
    m_listContact.append(carl);
    ui->listWidget->addItem( carl.toString() );
    //第4个
    Contact alice("Alice", "11111", "East");
    m_listContact.append(alice);
...
void Widget::on_pushButtonAdd_clicked()
{
    ...
    if(strName.isEmpty())
    {
        QMessageBox::warning(this,tr("姓名检查"),tr("姓名不能为空，请输入姓名！"));
        return;
    }
    //姓名不空
    Contact person(strName, strPhone, strAddress);
    //添加到列表对象和列表控件
    m_listContact.append(person);
    QListWidgetItem *item = new QListWidgetItem(person.toString()); //新建一个条目
    ui->listWidget->addItem(item);                                  //把条目加到列表控件中去
    //选中新条目
...
void Widget::on_pushButtonFind_clicked()
{
    ...
    //存在姓名
    //从列表对象查找序号
    int ix = m_listContact.indexOf( Contact(strName, "", "") );
    //如果找不到返回 -1
    if( ix < 0 )
    {
...
{
    //获取计数
    int nCount = m_listContact.count();
    if( nCount <= 1 )
    {
    ...
    {
        ui->listWidget->addItem( m_listContact[i].toString() );
    }
    //列表控件和列表对象的同步完毕
}
...
void Widget::on_pushButtonSave_clicked()
{
    ...
    //正常打开
    QDataStream ds( &fileOut );     //定义流
    //输出到文件只需一句
    ds<<m_listContact; //QList 和 Contact 都有配套的流操作符函数，因此可以这样一句搞定
    //保存成功
